...
enum xnn_status xnn_define_global_average_pooling_2d(
  ...
  uint32_t flags)
{
  if (!xnn_subgraph_xnnpack_initialized(xnn_node_type_global_average_pooling_2d)) {
  ...
  if (!xnn_subgraph_valid_input_id(xnn_node_type_global_average_pooling_2d, input_id, subgraph->num_values)) {
    return xnn_status_invalid_parameter;
  }
