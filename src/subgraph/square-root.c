  ...
  uint32_t flags)
{
  ...
  if (input_value->type != xnn_value_type_dense_tensor) {
    xnn_log_error(
      ...
      xnn_node_type_to_string(xnn_node_type_square_root), input_id, input_value->type);
    return xnn_status_invalid_parameter;
  }
  ...
  if (output_value->type != xnn_value_type_dense_tensor) {
    xnn_log_error(
      ...
      xnn_node_type_to_string(xnn_node_type_square_root), output_id, output_value->type);
    return xnn_status_invalid_parameter;
  }
