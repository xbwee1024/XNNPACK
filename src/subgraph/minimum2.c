...
enum xnn_status xnn_define_minimum2(
  ...
  uint32_t flags)
{
  if (!xnn_subgraph_xnnpack_initialized(xnn_node_type_minimum2)) {
    return xnn_status_uninitialized;
  }

  if (!xnn_subgraph_valid_nth_input_id(xnn_node_type_minimum2, input1_id, subgraph->num_values, 1)) {
  ...
  if (!xnn_subgraph_valid_nth_input_id(xnn_node_type_minimum2, input2_id, subgraph->num_values, 2)) {
    return xnn_status_invalid_parameter;
  }
