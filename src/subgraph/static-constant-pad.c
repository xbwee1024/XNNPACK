...
enum xnn_status xnn_define_static_constant_pad(
  ...
  uint32_t flags)
{
  if (!xnn_subgraph_xnnpack_initialized(xnn_node_type_static_constant_pad)) {
    return xnn_status_uninitialized;
  }

  if (!xnn_subgraph_valid_input_id(xnn_node_type_static_constant_pad, input_id, subgraph->num_values)) {
