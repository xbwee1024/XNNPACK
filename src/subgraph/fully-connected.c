...
enum xnn_status xnn_define_fully_connected(
  ...
  uint32_t flags)
{
  if (!xnn_subgraph_xnnpack_initialized(xnn_node_type_fully_connected)) {
  ...
  if (!xnn_subgraph_valid_input_id(xnn_node_type_fully_connected, input_id, subgraph->num_values)) {
    return xnn_status_invalid_parameter;
  }
