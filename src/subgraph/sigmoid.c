...
enum xnn_status xnn_define_sigmoid(
  ...
  uint32_t flags)
{
  if (!xnn_subgraph_xnnpack_initialized(xnn_node_type_sigmoid)) {
    return xnn_status_uninitialized;
  }

  if (!xnn_subgraph_valid_input_id(xnn_node_type_sigmoid, input_id, subgraph->num_values)) {
  ...
  if (input_value->type != xnn_value_type_dense_tensor) {
    xnn_log_error(
      ...
      xnn_node_type_to_string(xnn_node_type_sigmoid), input_id, input_value->type);
    return xnn_status_invalid_parameter;
  }
  ...
  if (output_value->type != xnn_value_type_dense_tensor) {
    xnn_log_error(
      ...
      xnn_node_type_to_string(xnn_node_type_sigmoid), output_id, output_value->type);
    return xnn_status_invalid_parameter;
  }
