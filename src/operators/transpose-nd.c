    ...
    xnn_operator_t* transpose_op_out)
{
  ...
  if (num_dims > XNN_MAX_TENSOR_DIMS) {
    xnn_log_error(
      ...
      xnn_operator_type_to_string(operator_type), num_dims, XNN_MAX_TENSOR_DIMS);
    goto error;
  }
  ...
  xnn_normalize_tensor_dimensions(num_dims, element_size, perm, shape, &normalized_dims, &context->element_size,
                                  normalized_perm, normalized_shape);

  size_t loop_order[XNN_MAX_TENSOR_DIMS];
  size_t n = normalized_shape[normalized_dims - 1];
  context->input_stride[normalized_dims - 1] = 1;
  context->output_stride[normalized_perm[normalized_dims - 1]] = 1;
